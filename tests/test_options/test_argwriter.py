...
class TestArgumentWriter(unittest.TestCase):
    ...
    def test_add_boolean_argument(self, mock_logger):
        ...
        # Create a mock schema with a boolean option
        test_schema = {
            "TEST_BOOL": {
                "arg": "--test-bool",
                "type": bool,
                "default": False,
                "help": "Test boolean option"
            }
        }
        ...
        args = self.parser.parse_args(['--test-bool'])
        self.assertTrue(args.test_bool)
        
        # Test default behavior (no flag provided)
        args = self.parser.parse_args([])
        self.assertIsNone(args.test_bool)
    ...
    def test_add_literal_argument(self, mock_logger):
        ...
        # Create a mock schema with a Literal option
        test_schema = {
            "TEST_LITERAL": {
                "arg": "--test-literal",
                "type": Literal["OPTION1", "OPTION2", "OPTION3"],
                "default": "OPTION1",
                "help": "Test literal option"
            }
        }
        ...
        # Parse valid choice
        args = self.parser.parse_args(['--test-literal', 'OPTION2'])
        self.assertEqual(args.test_literal, 'OPTION2')
        
        # Test invalid choice raises error
        with self.assertRaises(SystemExit):
            self.parser.parse_args(['--test-literal', 'INVALID'])
    ...
    def test_section_options_are_added(self, mock_logger):
        """Test that dependent options (using depends_on) are added to the parser."""
        test_schema = {
            "MAIN_OPTION": {
                "arg": "--main-option",
                "type": bool,
    ...
    def test_nested_section_options(self, mock_logger):
        """Test chained dependencies (using depends_on) are handled correctly."""
        test_schema = {
            "LEVEL_1": {
                "arg": "--level-1",
                "type": bool,
        ...
        # Parse chained dependent arguments
        args = self.parser.parse_args([
            '--level-1',
    ...
    def test_real_schema_integration(self, mock_logger):
        ...
        self.assertTrue(args.should_download_dependencies)
        self.assertEqual(args.steam_username, 'testuser')
